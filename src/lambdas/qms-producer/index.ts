...
import { deriveQMSEventType } from './derive-event-type';

const BOOTSTRAP_SERVERS = process.env.BOOTSTRAP_SERVERS!;
const TOPIC = process.env.MSK_TOPIC || 'aerospace.qms.events';
const DOMAIN = process.env.DOMAIN || 'QMS';
const SOURCE_SYSTEM = process.env.SOURCE_SYSTEM || 'qms-demo';
const ENTITY_TYPE = process.env.ENTITY_TYPE || 'NonConformance';
...
function processRecord(record: DynamoDBRecord) {
  ...
  const eventType = deriveQMSEventType(newImage, oldImage);
  if (!eventType) return null;

  const eventId = randomUUID();
  const entityId = newImage.ncrId || newImage.capaId || newImage.mrbId
    || newImage.faiId || newImage.inspectionId || newImage.PK;
  ...
  return {
    ...
    actor: {
      userId: newImage.raisedBy || newImage.lastModifiedBy || 'system',
