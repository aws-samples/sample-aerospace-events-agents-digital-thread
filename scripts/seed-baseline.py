...
# Deterministic seed → every SEED-* id is stable across runs, so reset → seed
# reproduces an identical baseline (same records, same graph) every time.
random.seed(20260601)

# Committed, byte-identical PLM drawings (config/drawings/), uploaded to the datalake
# so the demo's drawing artifacts are reproducible without live generation.
# Regenerate with scripts/generate-seed-drawings.py if drawing.py changes.
DRAWINGS_DIR = Path(__file__).resolve().parents[1] / 'config' / 'drawings'
...
# 44821-003 carries its full A→B→C history: the bore tolerance is tightened at rev C
# (+0.05 → +0.02 H7), which is what makes a Ø25.04 bore out-of-tolerance. The latest
# rev is RELEASED; superseded revs are SUPERSEDED. The committed PNG for each rev lives
# at config/drawings/{partNumber}/{drawingNumber}-{rev}.png.
SEED_DRAWINGS = {
    '44821-003': [
...
def generate_plm_history(start: datetime, end: datetime) -> list:
    ...
    for pn in PART_NUMBERS:
        ...
        # (uploaded to S3 in main()) so the demo's drawings are reproducible from seed
        # alone. 44821-003 carries 3 revs (A/B/C) showing the bore-tolerance tightening.
    ...
    for i in range(5):
        ...
        # Change notice cuts the new revision in.
        ecn = f'ECN-SEED-{i+1}'
        items.append({
...
def upload_seed_drawings():
    """Ensure the committed PLM drawing PNGs are in the datalake drawings/ prefix.

    The drawings are normally deployed to S3 by StorageStack (a CDK BucketDeployment
    of config/drawings/), so this is a best-effort top-up that also lets the CLI seed
    work against a freshly-wiped bucket. Idempotent — same bytes, same keys."""
    if not DRAWINGS_DIR.exists():
        return
    s3 = boto3.client('s3', region_name=REGION)
    n = 0
    for pn, revs in SEED_DRAWINGS.items():
        for dwg, rev, _tol, _maturity in revs:
            local = DRAWINGS_DIR / pn / f'{dwg}-{rev}.png'
            if not local.exists():
                continue
            s3.put_object(Bucket=DATALAKE_BUCKET, Key=f'drawings/{pn}/{dwg}-{rev}.png',
                          Body=local.read_bytes(), ContentType='image/png')
            n += 1
    if n:
        print(f'    {n} drawings → s3://{DATALAKE_BUCKET}/drawings/')
...
def main():
    ...
    print('\n  Ensuring committed PLM drawings in S3...')
    upload_seed_drawings()
